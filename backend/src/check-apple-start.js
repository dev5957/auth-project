...
const { PENDING_PHONE_PLACEHOLDER } = require('./services/oauthService');

const TEST_SECRET = 'apple-start-test-secret-not-for-production';
const TEST_ISSUER = 'auth-project';
const TEST_AUDIENCE = 'auth-project-app';
const TEST_PORT = 30446;
const IDENTITY_TOKEN = 'apple-identity-token-must-not-appear-in-logs';
...
function createMemoryDb({ users = [], tokens = [], verifications = [] } = {}) {
  ...
  async function query(sql, params = []) {
    ...
    if (key.includes('FROM USERS') && key.includes("AUTH_PROVIDER = 'APPLE'") && key.includes('PROVIDER_USER_ID')) {
      const row = state.users.find(
        (user) => user.auth_provider === 'apple' && user.provider_user_id === params[0]
      );
    ...
    if (key.startsWith('INSERT INTO USERS')) {
      ...
      throw new Error('users must not be inserted by apple/start');
    }
...
function startTestServer(port) {
  ...
  const env = {
    ...
    JWT_EXPIRES_IN: '15m',
    APPLE_CLIENT_ID: 'apple-start-http-client-id',
...
async function main() {
  ...
  process.env.DATABASE_URL = previous.DATABASE_URL || 'postgres://apple-start-test/local';
  process.env.JWT_SECRET = TEST_SECRET;
  process.env.JWT_ISSUER = TEST_ISSUER;
  process.env.JWT_AUDIENCE = TEST_AUDIENCE;
  process.env.JWT_EXPIRES_IN = '15m';
  process.env.APPLE_CLIENT_ID = 'apple-start-unit-client-id';

  const identity = appleIdentity();

  await expectStatus(() => startAppleAuth({}), 400, 'identity_token is required');
  await expectStatus(() => startAppleAuth({ identity_token: '' }), 400, 'identity_token is required');
  await expectStatus(() => startAppleAuth({ identity_token: '   ' }), 400, 'identity_token is required');
  console.log('OK identity_token absent -> 400 via verifyAppleIdentityToken()');
  ...
  console.log('OK identity_token Apple invalide -> 401');
  ...
  let seenToken = null;
  const loggedIn = await startAppleAuth(
    { identity_token: IDENTITY_TOKEN },
    {
      db: existingDb,
      verifyAppleIdentityToken: async (identityToken) => {
        seenToken = identityToken;
        return appleIdentity({ email: null, first_name: null, last_name: null });
      },
    }
  );
  assert(seenToken === IDENTITY_TOKEN, 'existing: received identity_token');
  ...
  console.log('OK compte Apple existant sans email -> Login successful + session');

  const newDb = createMemoryDb({ users: [] });
  const pending = await startAppleAuth(
    { identity_token: IDENTITY_TOKEN, first_name: 'Ada', last_name: 'Lovelace' },
    {
      db: newDb,
      verifyAppleIdentityToken: async () => appleIdentity({ first_name: null, last_name: null }),
    }
  );
  ...
  assert(pending.provider === 'apple', 'new: provider');
  ...
  assert(newDb.state.verifications[0].registration_data.provider === 'apple', 'new: stored provider');
  assert(newDb.state.verifications[0].registration_data.provider_user_id === 'apple-user-123', 'new: provider_user_id');
  assert(newDb.state.verifications[0].registration_data.email === 'user@example.com', 'new: stored email');
  assert(newDb.state.verifications[0].registration_data.first_name === 'Ada', 'new: first_name from body');
  assert(newDb.state.verifications[0].registration_data.last_name === 'Lovelace', 'new: last_name from body');
  console.log('OK nouveau Apple -> oauth_verification_token, pas de users');

  const newWithoutEmailDb = createMemoryDb({ users: [] });
  await expectStatus(
    () =>
      startAppleAuth(
        { identity_token: IDENTITY_TOKEN },
        {
          db: newWithoutEmailDb,
          verifyAppleIdentityToken: async () => appleIdentity({ email: null }),
        }
      ),
    400,
    'email is required'
  );
  assert(newWithoutEmailDb.state.verifications.length === 0, 'new without email: no pending row');
  assert(newWithoutEmailDb.state.insertedUsers === 0, 'new without email: no users');
  console.log('OK nouveau Apple sans email -> 400');
  ...
  try {
    ...
    assert(missing.json && missing.json.error === 'identity_token is required', 'HTTP missing message');
    ...
    assert(!invalid.raw.includes(IDENTITY_TOKEN), 'HTTP response leaked identity_token');

    const combinedLogs = logs.join('');
    assert(!combinedLogs.includes(IDENTITY_TOKEN), 'HTTP logs leaked identity_token');
    console.log('OK HTTP POST /auth/apple/start et logs propres');
  } finally {
  ...
  console.log('Apple start checks succeeded.');
}

main().catch((err) => {
  console.error('Apple start checks failed:', err.message);
  process.exitCode = 1;
});
